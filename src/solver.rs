...
use crate::eval::{Penalties, Solution, eval_route, eval_solution};
...
fn candidate_vehicles(sol: &Solution) -> Vec<usize> {
...
pub fn first_solution(m: &Model) -> Solution {
    first_solution_with(m, |_| {})
}

/// `first_solution` reporting a single `FirstSolution` event at the end.
pub fn first_solution_with(m: &Model, mut log: impl FnMut(SearchEvent)) -> Solution {
    let nv = m.vehicle_count();
    let mut sol: Solution = vec![Vec::new(); nv];
        ...
        for (ui, &node) in unrouted.iter().enumerate() {
            for &v in &candidates {
                for pos in 0..=sol[v].len() {
                    ...
                    let Some(c) = eval_route(m, &Penalties::NONE, &scratch, VehicleId(v as u32))
                    else {
                        continue;
                    };
                    let delta = c - cost[v];
...
pub fn local_search(m: &Model, sol: &mut Solution) {
    local_search_with(m, sol, |_| {})
}

/// `local_search` reporting an `Improvement` per accepted move and a final
/// `Done`.
pub fn local_search_with(m: &Model, sol: &mut Solution, log: impl FnMut(SearchEvent)) {
    descend(m, &Penalties::NONE, sol, log)
}
...
fn descend(m: &Model, p: &Penalties, sol: &mut Solution, mut log: impl FnMut(SearchEvent)) {
    let mut cost: Vec<Cost> = (0..sol.len())
        .map(|v| eval_route(m, p, &sol[v], VehicleId(v as u32)).expect("infeasible start solution"))
        .collect();
    ...
    loop {
        ...
        while let Some(u) = queue.pop_front() {
            ...
            let (touched, operator) = match try_relocate(m, p, sol, &mut cost, u, r) {
                Some(v) => (vec![r, v], Operator::Relocate),
                None => match try_swap(m, p, sol, &mut cost, u, r) {
                    Some(v) => (vec![r, v], Operator::Swap),
                    None if try_two_opt(m, p, sol, &mut cost, r) => (vec![r], Operator::TwoOpt),
                    None => continue,
                },
            };
            improved = true;
        ...
        if !improved {
            ...
            for r in 0..sol.len() {
                ...
                if let Some(_v) = try_two_opt_star(m, p, sol, &mut cost, r) {
                    improved = true;
                    log(SearchEvent::Improvement {
...
pub fn guided_local_search(m: &Model, sol: &mut Solution, iters: usize) {
...
pub fn guided_local_search_with(
    ...
    let mut best_cost = eval_solution(m, sol).expect("infeasible local optimum");
    ...
    for iter in 1..=iters {
        ...
        descend(m, &penalties, sol, |_| {});

        // The descent just optimized penalized cost, which is not the cost we
        // rank solutions by. `eval_solution` is always the true one.
        let cost = eval_solution(m, sol).expect("infeasible after descent");
        if cost < best_cost {
            best_cost = cost;
            best.clone_from(sol);
...
fn penalize_worst_arcs(m: &Model, p: &mut Penalties, sol: &Solution) {
...
fn try_relocate(
    ...
    without.remove(at);
    let without_cost = eval_route(m, p, &without, VehicleId(r as u32))?;
    ...
    for v in candidate_vehicles(sol) {
        ...
        for pos in 0..=base.len() {
            ...
            let Some(c) = eval_route(m, p, &scratch, VehicleId(v as u32)) else {
                continue;
            };
            let delta = if v == r {
                c - cost[r]
...
fn try_swap(
    ...
    for v in 0..sol.len() {
        ...
        for q in 0..sol[v].len() {
            ...
            sol[r][at] = w;
            sol[v][q] = u;
            let new = eval_route(m, p, &sol[r], VehicleId(r as u32)).zip(eval_route(
                m,
                p,
                &sol[v],
                VehicleId(v as u32),
...
fn try_two_opt(m: &Model, p: &Penalties, sol: &mut Solution, cost: &mut [Cost], r: usize) -> bool {
    let len = sol[r].len();
    for i in 0..len {
        for j in i + 1..len {
            sol[r][i..=j].reverse();
            match eval_route(m, p, &sol[r], VehicleId(r as u32)) {
                Some(c) if c < cost[r] => {
                    cost[r] = c;
                    return true;
                }
        ...
        for i in 0..sol[r].len() {
            for j in 0..sol[v].len() {
                ...
                let new_v: Vec<NodeId> = sol[v][..=j]
                    ...
                    .copied()
                    .collect();
                let new = eval_route(m, p, &new_r, VehicleId(r as u32)).zip(eval_route(
                    m,
                    p,
                    &new_v,
                    VehicleId(v as u32),
...
mod tests {
    ...
    fn swap_finds_what_relocate_cannot() {
        ...
        assert_eq!(eval_solution(&m, &sol), Some(140));
        ...
        assert_eq!(eval_solution(&m, &sol), Some(120));
    }
    ...
    fn two_opt_star_repartitions_routes() {
        ...
        let mut cost: Vec<Cost> = (0..2)
            .map(|v| eval_route(&m, &Penalties::NONE, &sol[v], VehicleId(v as u32)).unwrap())
            .collect();
        assert_eq!(cost.iter().sum::<Cost>(), 80);

        let v = try_two_opt_star(&m, &Penalties::NONE, &mut sol, &mut cost, 0);
        assert_eq!(v, Some(1));
        assert_eq!(
            sol,
        ...
        assert_eq!(
            try_two_opt_star(&m, &Penalties::NONE, &mut sol, &mut cost, 0),
            None
        );
    }
